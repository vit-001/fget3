...
class GirlstopSite(TgpSite):
    @staticmethod
    def can_accept_url(url: URL) -> bool:
        return url.contain('girlstop.info/')

    @staticmethod
    def create_start_button(view:ViewManagerFromModelInterface):
        view.add_start_button(picture_filename='model/site/resource/picture/girlstop.png',
                              url=URL("http://www.girlstop.info/"))

    def get_shrink_name(self):
        return 'GT'

    def get_thumbs_containers(self, soup: BeautifulSoup) -> list:
        return soup.find_all('td', id='main_body_psto')

    def get_thumbs_from_container(self, container: BeautifulSoup) -> list:
        return container.find_all('td',{'class':'preview'})
