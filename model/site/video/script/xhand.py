...
class XhandSite(BaseSiteParser):
    @staticmethod
    def can_accept_url(url: URL) -> bool:
        return url.contain('xhand.com/')
    ...
    def create_start_button(view:ViewManagerFromModelInterface): #
        ...
        view.add_start_button(picture_filename='model/site/resource/xhand.svg',
                              ...
                              url=URL("https://xhand.com/latest-updates/", test_string='porn'))

    def get_shrink_name(self):
        return 'XH'

    def parse_thumbs(self, soup: BeautifulSoup, url: URL):
        contents=soup.find('div', {'class':'thumbs_list'})
        # pretty(contents)
        if contents:
            # pretty(contents)
